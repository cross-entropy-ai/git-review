...
def record_demo():
    ...
    with tempfile.TemporaryDirectory(prefix="git-review-demo-") as temporary:
        ...
        try:
            ...
            wait_for("0/8 viewed")
            time.sleep(2.5)
            caption("Spot the exact edit: deeper backgrounds highlight changed characters.")
            keys("f")
            keys("c", "o", "n", "f", "i", "g", "Enter")
            wait_for('"timeout": 30')
            time.sleep(4)
            caption("Character-level changes, with syntax colors, in both layouts.  [s]")
            keys("s")
            wait_for("DIFF · SPLIT")
            time.sleep(4)
            keys("s", "g")
            caption("See every file. Fold the noise.  [z]")
            keys("z")
            time.sleep(2)
            ...
            keys("m")
            wait_for("Review mode · fuzzy search")
            time.sleep(0.5)
